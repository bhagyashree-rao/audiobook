...
def audiobook(request):
    if 'loginuser' in request.session:
        ...
        # data, samplerate = sf.read(io.BytesIO(audioData.content))
        # print(data)
        s = io.BytesIO(audioData.content)
        audio = AudioSegment.from_file(s, format="raw",
                                   frame_rate=44100, channels=2,
                                   sample_width=2).remove_dc_offset()
        audio = audio - 3.5

        audio.export('text.mp3', format='mp3')
